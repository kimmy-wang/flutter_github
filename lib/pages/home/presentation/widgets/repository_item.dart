...
class RepositoryItem extends StatefulWidget {
  final int index;
  final TrendingModel repository;
  final bool last;
...
class _RepositoryItemState extends State<RepositoryItem> {
  ...
  Widget avatarImage() {
    return PhysicalModel(
      ...
      child: CircleAvatar(
        radius: 12,
        child: Image.network(widget.repository.avatar, width: 24, height: 24,),
      ),
    );
  }
  ...
  Widget get _stars {
    return const Icon(
      ...
      size: 16,
        ...
        .nestedPadding(padding: const EdgeInsets.only(right: 2))
        .addWidgetAsList(Text(
      _processCount(widget.repository.stars),
      style: const TextStyle(
        fontWeight: FontWeight.w500,
      ),
    ))
  ...
  Widget get _forks {
    return const Icon(
      ...
      size: 16,
        ...
        .nestedPadding(padding: const EdgeInsets.only(right: 2))
        .addWidgetAsList(Text(
      _processCount(widget.repository.forks),
      style: const TextStyle(
        fontWeight: FontWeight.w500,
      ),
    ))
  ...
  Widget _buildBy(BuildContext context) {
    var buildBys = <Widget>[];
    widget.repository.builtBy.asMap().forEach(
          (index, buildBy) {
  ...
  String _parseColorString() {
    var colorStr = widget.repository.languageColor;
    var color = "";
  ...
  String get getDescription {
    return widget.repository.description == null || widget.repository.description!.isEmpty
        ? 'No description given.'
        : widget.repository.description!;
  }

  String get getPrimaryLanguage {
    return widget.repository.language == null ||
        widget.repository.language!.isEmpty
        ? 'Unknown'
